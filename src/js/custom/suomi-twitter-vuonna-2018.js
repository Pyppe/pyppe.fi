$(function() {
  ...
  const asyncJson = suffix => {
    const tabId = $('#suomi-twitter-2018-tabs a.active').attr('href').replace('#','');
    const $tabContent = $(`#suomi-twitter-2018-tab-contents [data-tab-id="${tabId}"]`).hide();
    const $spinner = $('<i class="fa fa-spin fa-spinner fa-lg" style="margin-bottom: 50px;"></i>').insertBefore($tabContent);
    return $.get(`/dist/resources/twitter-2018/${suffix}.json`, {h: pyppe.resourceHash}).then(response => {
      $spinner.remove();
      $tabContent.show();
      return response;
    });
  ...
  function openInitialTab() {
    ...
    const availableTabs = $('#suomi-twitter-2018-tabs a').map((i, el) => $(el).attr('href').replace('#', ''));

    const initialTab = _.includes(availableTabs, view) ? view : 'yhteenveto';
    $(`#suomi-twitter-2018-tabs a[href="#${initialTab}"]`).click();
  }

  $('#suomi-twitter-2018-tabs a').click(function(e) {
    e.preventDefault();
    if ($(this).is('.disabled')) {
      return;
    }
    $('#suomi-twitter-2018-tabs a').removeClass('active');
    const id = $(this).addClass('active').attr('href').replace('#', '');
    pyppe.util.replaceUrlSearch({view: id === 'yhteenveto' ? null : id});

    $(`#suomi-twitter-2018-tab-contents [data-tab-id]`).hide();
    $(`#suomi-twitter-2018-tab-contents [data-tab-id="${id}"]`).show();
  ...
  function createUsersTabContent(rows) {
    ...
    const $container = $('#suomi-twitter-2018-users');
    ...
    const $userCanvas = $('<div id="suomi-twitter-vuonna-2018-users-chart"></div>').appendTo($container);
          ...
          render: row => {
            ...
            const hashtagsHtml = _.size(topHashtags) > 0 ? (
              _.map(topHashtags, hashtag => {
                const q = `#${hashtag} from:${user.screenName} since:2018-01-01 until:2019-01-01`;
          ...
          alterColumn: ($td, {activity}) => {
            bindPopover({
              ...
              table: _.map(activity, (count, idx) => tr(
                moment('2018-01-01').add(idx, 'month').format('YYYY MMMM'),
  ...
  function createHashtagsTabContent({hashtags}) {
    const $container = $('#suomi-twitter-2018-hashtags');
    ...
    pyppe.util.onWindowWidthResized($canvas, () => {
      d3.selectAll('#suomi-twitter-2018-hashtags .canvas svg').remove(); // remove existing
      drawHashtagCloud(hashtags, $canvas);
    });
    ...
    pyppe.util.createTableContent(
      ...
      [
        ...
        },
        {
          ...
          value: b => b.key,
          render: b => {
            const q = `#${b.key} since:2018-01-01 until:2019-01-01`
  ...
  function createTotalsTabContent(response) {
    ...
    $('#suomi-twitter-2018-totals-chart').highcharts({
      chart: { type: 'line' },
      title: { text: ' ' },
      xAxis: {
        type: 'datetime',
  ...
  function createViralsNavigation() {
    const $dayTable = $('#suomi-twitter-2018-virals table:first');
    const $bucketTable = $('#suomi-twitter-2018-virals table:eq(1)');
    ...
    bindTableSortingAndOnScrollEmbeddedTweets({
      $table: $bucketTable,
      $checkbox: $checkboxContainer.find('input'),
      $tweets: $('#suomi-twitter-2018-virals [data-twitter-id]')
    });
  }

  function createViralsDayTable(days) {
    const $container = $('#suomi-twitter-2018-virals');
