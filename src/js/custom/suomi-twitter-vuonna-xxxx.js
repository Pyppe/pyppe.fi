(function () {
  function bindActions(year) {
    ...
    const asyncJson = suffix => {
      const tabId = $(`#suomi-twitter-${year}-tabs a.active`).attr('href').replace('#','');
      const $tabContent = $(`#suomi-twitter-${year}-tab-contents [data-tab-id="${tabId}"]`).hide();
      const $spinner = $('<i class="fa fa-spin fa-spinner fa-lg" style="margin-bottom: 50px;"></i>').insertBefore($tabContent);
      return $.get(`/dist/resources/twitter-${year}/${suffix}.json`, {h: pyppe.resourceHash}).then(response => {
        $spinner.remove();
        $tabContent.show();
        return response;
      });
    ...
    function openInitialTab() {
      ...
      const availableTabs = $(`#suomi-twitter-${year}-tabs a`).map((i, el) => $(el).attr('href').replace('#', ''));
  
      const initialTab = _.includes(availableTabs, view) ? view : 'yhteenveto';
      $(`#suomi-twitter-${year}-tabs a[href="#${initialTab}"]`).click();
    }
  
    $(`#suomi-twitter-${year}-tabs a`).click(function(e) {
      e.preventDefault();
      if ($(this).is('.disabled')) {
        return;
      }
      $(`#suomi-twitter-${year}-tabs a`).removeClass('active');
      const id = $(this).addClass('active').attr('href').replace('#', '');
      pyppe.util.replaceUrlSearch({view: id === 'yhteenveto' ? null : id});
  
      $(`#suomi-twitter-${year}-tab-contents [data-tab-id]`).hide();
      $(`#suomi-twitter-${year}-tab-contents [data-tab-id="${id}"]`).show();
    ...
    function createUsersTabContent(rows) {
      ...
      const $container = $(`#suomi-twitter-${year}-users`);
      ...
      const $userCanvas = $(`<div id="suomi-twitter-vuonna-${year}-users-chart"></div>`).appendTo($container);
            ...
            render: row => {
              ...
              const hashtagsHtml = _.size(topHashtags) > 0 ? (
                _.map(topHashtags, hashtag => {
                  const q = `#${hashtag} from:${user.screenName} since:${year}-01-01 until:${year+1}-01-01`;
            ...
            alterColumn: ($td, {activity}) => {
              bindPopover({
                ...
                table: _.map(activity, (count, idx) => tr(
                  moment(`${year}-01-01`).add(idx, 'month').format('YYYY MMMM'),
    ...
    function createHashtagsTabContent({hashtags}) {
      const $container = $(`#suomi-twitter-${year}-hashtags`);
      ...
      pyppe.util.onWindowWidthResized($canvas, () => {
        d3.selectAll(`#suomi-twitter-${year}-hashtags .canvas svg`).remove(); // remove existing
        drawHashtagCloud(hashtags, $canvas);
      });
      ...
      pyppe.util.createTableContent(
        ...
        [
          ...
          },
          {
            ...
            value: b => b.key,
            render: b => {
              const q = `#${b.key} since:${year}-01-01 until:${year+1}-01-01`
    ...
    function createTotalsTabContent(response) {
      ...
      $(`#suomi-twitter-${year}-totals-chart`).highcharts({
        chart: { type: 'line' },
        title: { text: ' ' },
        xAxis: {
          type: 'datetime',
        ...
        tooltip: {
          ...
          formatter: function() {
            ...
            var peakRow = (function() {
              if (peak) {
                var percentage = decimalFormat(peak.count / bucket.tweetCount * 100);
    ...
    function createViralsNavigation() {
      const $dayTable = $(`#suomi-twitter-${year}-virals table:first`);
      const $bucketTable = $(`#suomi-twitter-${year}-virals table:eq(1)`);
      ...
      const $form = $(`
        <form class="form-inline">
          ...
          <div class="form-group form-check" style="margin-left: 20px; margin-top: 20px;">
            <input type="checkbox" class="form-check-input" id="exampleCheck1" />
            <label class="form-check-label" for="exampleCheck1">Näytä aihetwiitit</label>
          </div>
        </form>
      `).insertBefore($dayTable);
      $dayTable.hide();
      $bucketTable.hide();
      const $checkboxContainer = $form.find('.form-check');
      ...
      bindTableSortingAndOnScrollEmbeddedTweets({
        $table: $bucketTable,
        $checkbox: $checkboxContainer.find('input'),
        $tweets: $(`#suomi-twitter-${year}-virals [data-twitter-id]`)
      });
    }
  
    function createViralsDayTable(days) {
      const $container = $(`#suomi-twitter-${year}-virals`);
  ...
  pyppe.bindSuomiTwitterVuonnaXXXActions = bindActions
})()
