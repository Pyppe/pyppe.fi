$(function() {
  ...
  window.prepareForScreenshot = () => {
    ...
    $('.legend').hide();
    $('#twitter-graph-2016-09 button').hide();
  ...
  */
  ...
  const $container = $('#twitter-graph-2016-09');
      ...
      ${_.map(_.chain(Topics).map((t, key) => ({...t, key})).sortBy('shortLabel').value(), (t) => {
        return (`
          <span
            ...
            data-topic="${t.key}"
            tip-is-html
            tooltip-class="twitter-graph-2016-09-tooltip"
  ...
  _.forEach(_.sortBy(_.values(Topics), 'shortLabel'), t => {
    ...
    `).appendTo($('#twitter-graph-2016-09-topics'));
  });
  ...
  $.get('/dist/resources/twitter-graph-2016-09.json', {h: pyppe.resourceHash}).done(data => {
    ...
    `) : $(`<div class="twitter-graph-2016-09-user-option">${option.text}</div>`);
    graphData = data;
  ...
  function updateSearchUrlAndSelectedUser() {
    const search = _.compact([
      $container.is('.fullscreen') && 'fullscreen',
      openedUser && `user=${openedUser.screenName}`
    ]).join('&');
    pyppe.util.replaceUrlSearch(search);
  ...
  function drawGraph(inputData, fullscreen, isFirstLoad = false) {
    ...
    d3.selectAll("svg").remove(); // remove existing
    ...
    onNodeClick = d => {
      ...
      if (_.size(rows) > 0) {
        ...
        pyppe.util.bindTableSorting($table, 3);
      }
